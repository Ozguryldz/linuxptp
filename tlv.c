...
//result = auth_post_recv((struct authentication_tlv *) tlv);
static int auth_post_recv(struct authentication_tlv *aut)
{
	struct authentication_tlv *s;

	if(aut->length != 38){
		printf("This is not the correct received lenght:%u\n",aut->length);
		goto bad_length;
	}
	s = (struct authentication_tlv *) aut;
	printf("(After reception before nthons)TLV->ICV: ");
	for(int i = 0; i < sizeof(s->calc_ICV)/sizeof(*s->calc_ICV); i++)
		printf("%02x", s->calc_ICV[i]);
	printf("\n");
