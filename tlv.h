...
/**
 * @brief 
 * This part is added by me to provide AUTHENTICATION TLV suffix
 */
...
struct authentication_tlv {
	...
	UInteger16 length;
	UInteger8 spp;
	UInteger8 secParamIndicator; /*Boolean for DP(bit2), SP(bit1), RP(bit0) others are reserved*/
	UInteger32 keyID;
	/*UInteger32 disc_keyID; only applicable for delayed security processing*/
	Octet calc_ICV[32]; // 32:SHA256 HASH size. Must be defined somewhere like #define sizeSHA235 32
} PACKED;
/**
 * @brief 
 * Until This comment starting from the @brief is added by me
 */
